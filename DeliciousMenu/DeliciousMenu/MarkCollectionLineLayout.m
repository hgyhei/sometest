...
@implementation MarkCollectionLineLayout
- (instancetype)init
{
    if (self = [super init]) {
    }
...
- (void)prepareLayout
{
    ...
    // 每个cell的尺寸
    self.itemSize = CGSizeMake(MarkCollectionItemWidth, MarkCollectionItemWidth);
    CGFloat inset = (self.collectionView.frame.size.width - MarkCollectionItemWidth) * 0.5;
    self.sectionInset = UIEdgeInsetsMake(0, inset, 0, inset);
    // 设置水平滚动
    self.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    self.minimumLineSpacing = MarkCollectionItemWidth * 0.7;
...
//    NSArray *array = [super layoutAttributesForElementsInRect:rect];
    ...
    for (UICollectionViewLayoutAttributes *attrs in attributes) {
        ...
        // 差距越小, 缩放比例越大
        // 根据跟屏幕最中间的距离计算缩放比例
        CGFloat scale = 1 + MarkScaleFactor * (1 - (ABS(itemCenterX - centerX) / MarkActiveDistance));
